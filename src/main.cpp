...
unsigned int nTargetSpacing = 1 * 90; // Pentacoin - 1.5 minute blocks
...
unsigned int nModifierInterval = 10 * 60; // Pentacoin - time to elapse before new modifier is computed
...
const string strMessageMagic = "Pentacoin Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        strMiscWarning = strMessage;
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Pentacoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Regulating Bitcoin Legitimizes a Treacherous Business";
        ...
        vout.resize(1);
        vout[0].SetEmpty();
        CTransaction txNew(1, 1431523672, vin, vout, 0);
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1431523672;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 11477228;
       ...
       if (false && (block.GetHash() != hashGenesisBlock)) {
        ...
        assert(block.hashMerkleRoot == uint256("0x2d312fd5a3e63a7cf16d8d2c7901e2574887a1d467a090031612eda8dfb990b3"));
